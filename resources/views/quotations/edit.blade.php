<x-layout>
    <x-slot name="title">Edit Quotation - {{ $quotation->quotation_no }}</x-slot>
    ...
    <form id="edit-quotation-form">
        ...
        <div class="row mb-3">
            <div class="col-md-6">
                ...
                <select name="customer_custom_id" id="customer_select" class="form-control" required>
                    <option value="{{ $quotation->customer_custom_id }}" selected>
                        Loading customer...
                    </option>
                </select>
            </div>
            <div class="col-md-6">
                ...
                <select name="vehicle_no" id="vehicle_select" class="form-control">
                    @if($quotation->vehicle_no)
                        <option value="{{ $quotation->vehicle_no }}" selected>{{ $quotation->vehicle_no }}</option>
                    @endif
                </select>
            </div>
        </div>
        ...
        <div class="row mb-3">
            ...
            <div class="col-md-3 d-flex align-items-end">
                <button type="button" class="btn btn-primary w-100" id="add_item_btn">Add Item</button>
            </div>
        </div>
    ...
    @push('scripts')
        <script>
            ...
            let selectedCustomerId = '{{ $quotation->customer_custom_id }}';
            let selectedVehicle = '{{ $quotation->vehicle_no }}';
            ...
            function loadCustomerData() {
                // Load customer info
                $.get('{{ route('quotations.customer_search') }}', {
                    term: '{{ $quotation->customer_custom_id }}'
                }).done(function(data) {
                    if (data.length > 0) {
                        const customer = data.find(c => c.id === '{{ $quotation->customer_custom_id }}');
                        if (customer) {
                            $('#customer_select').html(`<option value="${customer.id}" selected>${customer.text}</option>`);
                        }
                    }
                });
            }
            ...
            function loadExistingItems() {
                $.get('{{ route('quotations.get_edit_session_items') }}')
            ...
            function setupItemSearch() {
                $('#item_selector').select2({
                    ...
                    ajax: {
                        url: selectedType === 'spare' ? '{{ route('quotations.item_search') }}' : '{{ route('quotations.job_search') }}',
                        dataType: 'json',
                        delay: 250,
                        processResults: data => ({results: data}),
                    }
            ...
            $('#add_item_btn').on('click', function() {
                ...
                fetch(`{{ route('quotations.add_edit_temp_item') }}`, {
                    method: 'POST',
                    headers: {
            ...
            function removeItem(index) {
                fetch(`{{ route('quotations.remove_edit_temp_item') }}`, {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        'X-CSRF-TOKEN': '{{ csrf_token() }}'
            ...
            $('#edit-quotation-form').on('submit', function(e) {
                e.preventDefault();
                const customer_id = $('#customer_select').val();
                const vehicle_no = $('#vehicle_select').val();

                fetch(`{{ route('quotations.update', $quotation->id) }}`, {
                    method: 'PUT',
                    headers: {
                        'Content-Type': 'application/json',
                        'X-CSRF-TOKEN': '{{ csrf_token() }}'
                    ...
                    if (data.success) {
                        ...
                        Swal.fire('Error', data.message || 'Failed to update quotation.', 'error');
                    }
                });
            });
        </script>
